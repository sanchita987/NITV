...
export class PaymentUpdateComponent {
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(params => {
      const id = params['id'];
      if (id) {
        this.data.getPayment(id).subscribe((response: any) => {
          console.log('Fetched payment by ID', response);
          if (response && response.data) {
            this.patchPaymentDetails(response.data); // Patch the payment details
          }
        });
      }
    });
  ...
  patchPaymentDetails(paymentDetails: any) {
    // Formatting the date string
    const formattedDate = this.datePipe.transform(paymentDetails.paid_at, 'yyyy-MM-ddTHH:mm');
  
    // Patching the payment details into the form
    this.paymentForm.patchValue({
      customer: paymentDetails.customer.first_name,
      customer_id: paymentDetails.customer.id,
      mode: paymentDetails.mode,
      amount: paymentDetails.amount,
      send_mail: paymentDetails.send_mail,
      paid_at: formattedDate, 
      reference: paymentDetails.payment_number,
      description: '', 
      apply_to_invoices: '',
      amount_received: '',
